...
# Keep $SECRETS_DIR at the default 0755: the $ACTUAL_USER FL-server child must
# be able to TRAVERSE it to read its key at $SECRETS_DIR/grpc/server.key, so this
# directory must NOT inherit the tightened umask used for the secrets file below.
mkdir -p "$SECRETS_DIR"
if [ ! -f "$SECRETS_FILE" ]; then
  echo "      Generating new secrets → $SECRETS_FILE"
  # Create the file 0600 BEFORE writing so secrets are never world/group-readable,
  # not even transiently. umask 077 around the write closes any race window — it
  # forces every file created in this block to 0600 from birth even if the explicit
  # `install -m 600` were ever dropped. Saved/restored so it stays scoped to the
  # write and never tightens $SECRETS_DIR (see the note above).
  _prev_umask="$(umask)"
  umask 077
...
# SE-7 trust-domain isolation: a DISTINCT secret for the network-facing FL connection tokens, so a
# compromise of the FL boundary can't forge web-session JWTs (and vice versa). Without this,
# app.fl.token-secret silently falls back to app.jwt.secret and the two domains share one key.
APP_FL_TOKEN_SECRET=$(openssl rand -hex 64)
...
SECRETS
  umask "$_prev_umask"   # restore: later steps (nginx/unit writes) want default perms
else
...
EOF
# The unit carries NO inline secrets (they live in the 0600 EnvironmentFile
# above), but lock it to 0600 root:root anyway — defense-in-depth so any future
# inline Environment= secret could never leak to other local users. systemd runs
# as root (PID 1) and reads units directly, so 0600 does not affect management
# (systemctl status/enable still work — they query systemd over D-Bus).
chmod 600 /etc/systemd/system/fedlearn.service
