...
USAGE
    PYTHONPATH=src python3 run_secure_agg_live_test.py [--rounds N] [--clients N] [--dropout]
...
# ---------------------------------------------------------------------------------------------
def _run_server(port, rounds, num_clients, secure, threshold, secret_b64, result_q, tag):
...
# ---------------------------------------------------------------------------------------------
def _run_client(port, partition, client_id, token, result_q, tag, quit_after=None):
...
# One run
# ---------------------------------------------------------------------------------------------
def run_once(rounds, num_clients, secure, threshold, tag, timeout_s=180):
    ...
    server = ctx.Process(
        target=_run_server,
        args=(port, rounds, num_clients, secure, threshold, secret_b64, server_q, tag),
    )
    clients = [
        ctx.Process(
            target=_run_client,
            args=(port, p, f"c{p}", _mint_token(secret_b64, p, f"c{p}"), client_q, tag),
        )
...
def main() -> int:
    ...
    print(f"\n=== SECURE run: {args.clients} clients, {args.rounds} rounds, "
    ...
    ok = _run_succeeded(secure, args.clients) and _audit_server_log(
        "secure", expect_secure=True, rounds=args.rounds)

    if not args.secure_only:
        print(f"\n=== PLAINTEXT control run (same seed, auth ON) ===")
        t0 = time.time()
        ...
        ok = ok and _run_succeeded(plain, args.clients) and _audit_server_log(
            "plain", expect_secure=False, rounds=args.rounds)
...
def _audit_server_log(tag, expect_secure, rounds) -> bool:
    ...
    if not os.path.exists(path):
        print(f"  FAILED: no server log at {path}")
        return False
    ...
    refused_identity = text.count("Secure aggregation requires verified client identity")
    downgrades = text.count("Refusing plaintext submission")

    print(f"  audit: masked={masked} plaintext={plaintext} secure_rounds={secure_rounds} "
          f"identity_refusals={refused_identity} downgrade_refusals={downgrades}")
    ...
    if expect_secure:
        if masked == 0:
            ...
            return False
        if plaintext:
            print(f"  FAILED: {plaintext} plaintext submission(s) reached the aggregation on a "
                  f"secure server")
            return False
        if secure_rounds < rounds:
            print(f"  FAILED: {secure_rounds} secure round(s) completed, expected {rounds}")
            return False
    else:
        if masked:
            print(f"  FAILED: {masked} masked submission(s) on a plaintext server")
            return False
        if plaintext == 0:
