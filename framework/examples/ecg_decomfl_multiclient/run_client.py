# =============================================================================
# run_client.py - DeComFL Client Implementation (Algorithm 4) - Multi-Client
# =============================================================================
...
class DeComFLClient:
    ...
    def rebuild_model(self, rebuild_seeds: List[List[List[int]]],
                      rebuild_grads: List[List[List[float]]],
                      learning_rate: float):
        """
        Rebuild model from missed rounds (Algorithm 2, Lines 2-9)

        Args:
            rebuild_seeds: Seeds from missed rounds
            rebuild_grads: Average gradient scalars from missed rounds
            learning_rate: Learning rate
        """
        print(f"[CLIENT {self.client_id}] Rebuilding model from {len(rebuild_seeds)} missed rounds")

        for round_seeds, round_grads in zip(rebuild_seeds, rebuild_grads):
            for k, (k_seeds, k_grads) in enumerate(zip(round_seeds, round_grads)):
                delta = torch.zeros_like(self.x_current)

                for p, (seed, g) in enumerate(zip(k_seeds, k_grads)):
                    # Regenerate perturbation from seed
                    z = self._generate_perturbation(seed)
                    # Accumulate update
                    delta += g * z
        ...
        # Update model with rebuilt parameters
