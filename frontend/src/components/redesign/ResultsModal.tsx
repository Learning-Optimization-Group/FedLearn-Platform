    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-slate-950/60 backdrop-blur-sm font-sans">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-6xl h-full max-h-[90vh] rounded-md shadow-2xl shadow-cyan-900/10 flex flex-col overflow-hidden text-slate-200">
        ...
        <div className="flex items-center justify-between p-8 pb-6 border-b border-slate-800">
          <div>
            <h2 className="text-[24px] font-semibold tracking-tight text-slate-100">{projectName} — Results</h2>
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-slate-400 bg-slate-800 hover:bg-slate-700 rounded-sm transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-8 flex flex-col gap-6 scroll-smooth">
          ...
          {!hasResults && (
            <div className="flex flex-col items-center justify-center flex-1 text-center gap-3 py-16">
              ...
              <h3 className="text-[20px] font-semibold tracking-tight text-slate-200">No results yet</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-800/40 rounded-md p-8 border border-slate-700/50">
              <div className="flex items-center gap-2 text-green-500 mb-3">
                ...
                <span className="font-semibold tracking-wide uppercase text-[12px] text-slate-400">Best Accuracy</span>
              </div>
            ...
            <div className="bg-slate-800/40 rounded-md p-8 border border-slate-700/50">
              <div className="flex items-center gap-2 text-rose-500 mb-3">
                ...
                <span className="font-semibold tracking-wide uppercase text-[12px] text-slate-400">Final Loss</span>
              </div>
              <div className="text-[36px] font-semibold tracking-tighter text-slate-100">
                {finalLoss.toFixed(4)}
              </div>
            ...
            <div className="bg-slate-800/40 rounded-md p-8 border border-slate-700/50">
              <div className="flex items-center gap-2 text-cyan-500 mb-3">
                ...
                <span className="font-semibold tracking-wide uppercase text-[12px] text-slate-400">Total Rounds</span>
              </div>
              <div className="text-[36px] font-semibold tracking-tighter text-slate-100">
                {results.length}
              </div>
          ...
          <div className="flex items-center gap-3 border-b border-slate-800 pb-2">
            <button
              ...
              className={cn(
                ...
                activeTab === 'chart' ? "bg-slate-800 text-cyan-400 border border-slate-700" : "text-slate-400 hover:bg-slate-800 hover:text-slate-200 border border-transparent"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-2 px-5 py-2.5 rounded-md text-[14px] font-medium tracking-tight transition-all duration-200",
                activeTab === 'table' ? "bg-slate-800 text-cyan-400 border border-slate-700" : "text-slate-400 hover:bg-slate-800 hover:text-slate-200 border border-transparent"
              )}
            >
                ...
                <div className="flex-1 min-h-[400px]">
                  <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                    <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                      ...
                      <XAxis dataKey="round" stroke="#475569" tick={{ fill: '#94a3b8', fontSize: 12 }} tickLine={false} axisLine={false} dy={10} />
                      <YAxis yAxisId="left" stroke="#475569" tick={{ fill: '#94a3b8', fontSize: 12 }} tickLine={false} axisLine={false} dx={-10} />
                      <YAxis yAxisId="right" orientation="right" stroke="#475569" tick={{ fill: '#94a3b8', fontSize: 12 }} tickLine={false} axisLine={false} dx={10} tickFormatter={(v: number) => `${(v * 100).toFixed(0)}%`} />
                      <Tooltip
                        contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '6px', color: '#f1f5f9', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.5)' }}
                      ...
                      />
                      <Line yAxisId="left" type="monotone" dataKey="loss" stroke="#f43f5e" strokeWidth={2.5} dot={false} activeDot={{ r: 5, strokeWidth: 0, fill: '#f43f5e' }} />
                      <Line yAxisId="right" type="monotone" dataKey="accuracy" stroke="#22c55e" strokeWidth={2.5} dot={false} activeDot={{ r: 5, strokeWidth: 0, fill: '#22c55e' }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="h-[450px] overflow-auto rounded-md border border-slate-800">
                <table className="w-full text-left border-collapse bg-slate-900">
                  <thead className="bg-slate-800 sticky top-0 z-10 shadow-sm border-b border-slate-700">
                    <tr>
                      <th className="px-6 py-3 text-[12px] font-semibold uppercase tracking-wider text-slate-400">Round</th>
                      <th className="px-6 py-3 text-[12px] font-semibold uppercase tracking-wider text-slate-400">Loss</th>
                      <th className="px-6 py-3 text-[12px] font-semibold uppercase tracking-wider text-slate-400">Accuracy</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800 text-[13px] font-mono tracking-tight text-slate-300">
                    {results.slice().reverse().map((row) => (
                      <tr key={row.serverRound} className="hover:bg-slate-800/50 transition-colors">
                        <td className="px-6 py-3.5 text-slate-400">#{row.serverRound}</td>
                        <td className="px-6 py-3.5 text-rose-400">{row.loss.toFixed(4)}</td>
                        <td className="px-6 py-3.5 text-green-400">{(row.accuracy * 100).toFixed(2)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
