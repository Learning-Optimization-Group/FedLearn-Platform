...
This mirrors the C++ mobile release gate `mobile_client/shared/tests/rng_parity_test.cpp` (`ASSERT_NEAR(..., 1e-6f)`). A companion check, `test_torch_version_matches_manifest`, guards against an unintentional `torch` bump silently changing the contract — re-freezing the fixture must be deliberate, with the new `torch_version` recorded in the manifest so the C++ parity gate re-validates.
