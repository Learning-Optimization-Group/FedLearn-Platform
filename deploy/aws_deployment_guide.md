> **Status:** moved here from the untracked `docs/guides/` when rb merged into main, so it travels
> with the deploy assets. It was written against an earlier build and parts are stale: the platform
> runs PostgreSQL in every profile (H2 is retired), the ECS Fargate path described below was removed
> (OP-14 — setting `ecs.cluster-name` now fails the boot), and `FlowerServerManager` is now
> `FlServerManager` (DA-12). The single-host EC2 flow itself still applies. See `deploy/TLS.md` for
> the TLS posture and `scripts/ec2-bootstrap.sh` for what bootstrap automates.
...
export EC2_HOST=<your-ec2-public-ipv4>  # from the EC2 console, e.g. 54.123.45.67
