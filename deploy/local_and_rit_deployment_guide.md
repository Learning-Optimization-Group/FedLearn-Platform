...
> **Status:** moved here from the untracked `docs/guides/` when rb merged into main. It was written
> against an earlier build: the `flwr-datasets` install step below is obsolete, since flwr was
> dropped (`65048b6`) and `fl-runtime/recipes.py` reproduces the CIFAR-10 shard natively. The
> local/VPN topology and the client launch flow still apply.
...
spring.datasource.password=${SPRING_DATASOURCE_PASSWORD:your-local-db-password}
